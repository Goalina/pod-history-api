...
- 多机 CI 场景下由 LWS / Volcano Job 等控制器创建的 worker pod，会自动关联到拉起它的 job pod，继承相同的 PR / 工作流信息（`extend_env_comments`）与 `source`。双路匹配策略：LWS pod 优先用 `BENCHMARK_JOB_NAME` 与 `job_display_name` 精确匹配（vllm-ascend 场景），无 `BENCHMARK_JOB_NAME` 时 fallback 到 token 匹配（sglang Volcano 等）；候选不唯一时保持 `unknown`
